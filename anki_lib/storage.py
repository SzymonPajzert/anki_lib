...
from anki_lib.utils import intTime, isWin
from anki_lib.db import DB
from anki_lib.collection import _Collection
from anki_lib.consts import *
from anki_lib.stdmodels import addBasicModel, addClozeModel, addForwardReverse, \
...
def _upgrade(col, ver):
    ...
    if ver < 6:
        col.modSchema(check=False)
        import anki_lib.models
        for m in col.models.all():
            m['css'] = anki_lib.models.defaultModel['css']
            for t in m['tmpls']:
                if 'css' not in t:
