...
from anki_lib.utils import checksum, isWin, isMac
from anki_lib.db import DB, DBError
from anki_lib.consts import *
from anki_lib.latex import mungeQA
...
class MediaManager:
    ...
    def __init__(self, col, server):
        ...
        try:
            ...
            print(err)
            raise Exception("invalidTempFolder: {}".format(self._dir))
        # change database
        self.connect()
...
""")
    ...
    def _expandClozes(self, string):
        ...
        strings = []
        from anki_lib.template.template import clozeReg
        def qrepl(m):
            if m.group(4):
                return "[%s]" % m.group(4)
            else:
