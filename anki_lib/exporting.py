...
from anki_lib.utils import ids2str, splitFields, namedtmp, stripHTML
from anki_lib.hooks import runHook
from anki_lib.import Collection
...
class AnkiPackageExporter(AnkiExporter):
    ...
    def doExport(self, z, path):
        # export into the anki_lib. file
        colfile = path.replace(".apkg", ".anki_lib.")
        AnkiExporter.exportInto(self, colfile)
        if not self._v2sched:
            z.write(colfile, "collection.anki_lib.")
        else:
            ...
            z.write(colfile, "collection.anki_lib.1")
