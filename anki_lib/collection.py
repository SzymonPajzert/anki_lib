...
from anki_lib.utils import ids2str, fieldChecksum, \
    intTime, splitFields, joinFields, maxID, devMode, stripHTMLMedia
from anki_lib.hooks import  runFilter, runHook
from anki_lib.models import ModelManager
from anki_lib.media import MediaManager
from anki_lib.decks import DeckManager
from anki_lib.tags import TagManager
from anki_lib.consts import *
from anki_lib.errors import AnkiError
from anki_lib.sound import stripSounds
import anki_lib.latex # sets up hook
import anki_lib.cards
import anki_lib.notes
import anki_lib.template
import anki_lib.find
...
class _Collection:
    ...
    def __init__(self, db, server=False, log=False):
        ...
        self.path = db._path
        self._openLog()
        self.log(self.path, anki_lib.version)
        self.server = server
    ...
    def _loadScheduler(self):
        ver = self.schedVer()
        if ver == 1:
            from anki_lib.sched import Scheduler
        elif ver == 2:
            from anki_lib.schedv2 import Scheduler
...
crt=?, mod=?, scm=?, dty=?, usn=?, ls=?, conf=?""",
    ...
    def reopen(self):
        ...
        import anki_lib.db
        if not self.db:
            self.db = anki_lib.db.DB(self.path)
            self.media.connect()
            self._openLog()
    ...
    def getCard(self, id):
        return anki_lib.cards.Card(self, id)

    def getNote(self, id):
        return anki_lib.notes.Note(self, id=id)
    ...
    def newNote(self, forDeck=True):
        ...
        return anki_lib.notes.Note(self, self.models.current(forDeck))
...
%s""" % where)
    ...
    def findCards(self, query, order=False):
        return anki_lib.find.Finder(self).findCards(query, order)

    def findNotes(self, query):
        return anki_lib.find.Finder(self).findNotes(query)

    def findReplace(self, nids, src, dst, regex=None, field=None, fold=True):
        return anki_lib.find.findReplace(self, nids, src, dst, regex, field, fold)

    def findDupes(self, fieldName, search=""):
        return anki_lib.find.findDupes(self, fieldName, search)

    # Stats
    ##########################################################################

    def cardStats(self, card):
        from anki_lib.stats import CardStats
        return CardStats(self, card).report()

    def stats(self):
        from anki_lib.stats import CollectionStats
...
and type=0""", [intTime(), self.usn()])
    ...
    def _openLog(self):
        if not self._debugLog:
            return
        lpath = re.sub(r"\.anki_lib.$", ".log", self.path)
