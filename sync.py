...
from anki_lib.db import DB, DBError
from anki_lib.utils import ids2str, intTime, platDesc, checksum, devMode
from anki_lib.consts import *
from anki_lib.utils import versionWithBuild
...
class HttpSyncer:
    ...
    def syncURL(self):
        if devMode:
            url = "https://l1sync.anki_lib.eb.net/"
        else:
    ...
    def meta(self):
        ...
        ret = self.req(
            "meta", io.BytesIO(json.dumps(dict(
                v=SYNC_VER, cv="anki_lib.esktop,%s,%s"%(versionWithBuild(), platDesc()))).encode("utf8")),
            badAuthRaises=False)
        if not ret:
            # invalid auth
            return
...
class FullSyncer(HttpSyncer):
    ...
    def __init__(self, col, hkey, client, hostNum):
        ...
        self.postVars = dict(
            k=self.hkey,
            v="anki_lib.esktop,%s,%s"%(anki_lib.version, platDesc()),
        )
        self.col = col
